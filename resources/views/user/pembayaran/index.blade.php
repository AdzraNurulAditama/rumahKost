...
<div class="max-w-5xl mx-auto px-4">
    ...
    <div class="bg-white rounded-2xl shadow p-5 mb-8 flex gap-4 items-center">
        @if($sewa->kost)
            ...
            <div>
                <h3 class="font-bold text-lg">{{ $sewa->kost->nama }}</h3>
                <p class="text-sm text-gray-500">{{ $sewa->kost->alamat }}</p>
    ...
    <div class="bg-white p-6 rounded-2xl shadow max-w-xl mx-auto text-center">
        ...
        <h2 class="text-xl font-bold mb-6">Bayar Sewa Pertama</h2>

        {{-- Tampilkan error jika token gagal didapat --}}
        @if($errorMessage ?? null)
            ...
            <button
                ...
                class="w-full bg-gray-200 hover:bg-gray-300 text-gray-700 py-3 rounded-xl font-semibold transition">
                🔄 Coba Lagi
            </button>
        @else
            <p class="text-gray-500 mb-6">Klik tombol di bawah untuk melakukan pembayaran</p>
            <button
                id="pay-button"
                onclick="payNow()"
...
{{-- Load Snap.js hanya jika token tersedia --}}
@if($snapToken)
<script
    ...
    function payNow() {
        ...
        snap.pay(snapToken, {
            ...
            onPending: function(result) {
                ...
                alert('Pembayaran sedang diproses. Silakan selesaikan pembayaran sesuai instruksi.');
                btn.disabled = false;
